...
#**********************Correlation Attack Implementation***********************
#Byte key guess
maxcpa = [0] * 256
...
for kguess in tnrange(0, 256):
    hws = np.array([[HW[aes_internal(textin[0], kguess)]
...
for bnum in tnrange(0, 16):
    ...
    for kguess in range(0, 256):
        hws = np.array([[HW[aes_internal(textin[bnum], kguess)]
                         for textin in textin_array]]).transpose()
        hws_bar = mean(hws)
        o_hws = std_dev(hws, hws_bar)
        correlation = cov(trace_array, t_bar, hws, hws_bar)
...
for bnum in range(16):
    assert bestguess[bnum] == key[bnum], \
        "Byte {} failed, expected {:02X} got {:02X}".format(
            bnum, key[bnum], bestguess[bnum])
print("✔️ OK to continue!")
